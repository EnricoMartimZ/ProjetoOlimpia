...
| POST | `/pesquisas/{id}/edicoes` | Lança edição (auto-incrementa número; encerra outras ativas) | servidor |
| POST | `/edicoes/{id}/status` | Ativa/encerra edição (`acao`); ao ativar, encerra outras ativas | servidor |
| DELETE | `/edicoes/{id}` | Exclui edição (cascata) | servidor |
